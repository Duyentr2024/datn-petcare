...
const OrderOffline = () => {
  ...
  const ProductLists = ({ products, handleAddProduct, selectedProducts, currentPage, setCurrentPage }) => {
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center bg-white w-full rounded px-2 py-1 mb-1 border border-[#2e7d32] focus-within:ring-1 focus-within:ring-[#2e7d32]/50 focus-within:border-[#2e7d32]">
          <IoSearchOutline className="text-[#2e7d32] text-sm mr-1" />
          <input
            type="text"
        ...
        <div className="bg-[#e8f5e9] rounded-md mb-1 p-1">
        ...
        <div className="overflow-auto flex-1 rounded-md border border-[#2e7d32]/20">
                ...
                >
                    ...
                    />
                      ...
                      <td className="p-1 text-center">
                        {isSelected ? (
                          <div className="bg-[#2e7d32] rounded-full w-4 h-4 mx-auto flex items-center justify-center">
                    ...
                    {localSearchTerm ?
                      <div className="flex flex-col items-center py-2">
                        ...
                        <button
                          ...
                          className="mt-1 text-[#2e7d32] hover:underline"
                        >
                          Xóa tìm kiếm
                        </button>
                      </div> :
        ...
        <div className="flex justify-between items-center pt-1">
          <button
            className="px-1.5 py-0.5 text-[10px] bg-[#e8f5e9] text-[#2e7d32] rounded hover:bg-[#2e7d32]/20 disabled:opacity-50 flex items-center"
          ...
          >
            Sau
            <svg className="w-2.5 h-2.5 ml-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  // Empty implementations of tab management functions
  const addNewTab = () => {
    if (tabs.length >= 3) {
      toast.warning("Chỉ được tạo tối đa 3 hóa đơn cùng lúc!");
      return;
    }

    // Always create tabs with fixed IDs 1, 2, 3 in order
    const existingIds = tabs.map(tab => tab.id);
    let newTabId = 1;
    if (!existingIds.includes(1)) newTabId = 1;
    else if (!existingIds.includes(2)) newTabId = 2;
    else if (!existingIds.includes(3)) newTabId = 3;

    const newTab = {
      id: newTabId,
      title: `Hóa đơn ${newTabId}`,
      products: [],
      customerPayment: 0,
      inputPayment: '',
      change: 0,
    ...
    // Sort tabs by ID to maintain order 1, 2, 3
    const newTabs = [...tabs, newTab].sort((a, b) => a.id - b.id);
    setTabs(newTabs);
  ...
  const removeTab = (tabId) => {
    if (tabs.length <= 1) {
      toast.warning("Phải giữ ít nhất một hóa đơn!");
      return;
    }
    ...
    // If active tab is removed, set active tab to the first tab by ID
    if (activeTab === tabId) {
      const lowestIdTab = filteredTabs.reduce((lowest, tab) =>
        tab.id < lowest.id ? tab : lowest, filteredTabs[0]);
      setActiveTab(lowestIdTab.id);
    }
  };

  return (
    <div className="flex flex-col">
      {/* Main layout with top padding for staff header */}
      <div className="flex flex-col h-full">
        {/* Order header with tabs */}
        <header className="p-1 border-b shadow-sm">
            ...
            {tabs.map(tab => (
              ...
              >
                ...
                {tabs.length < 3 && (
                  <button
                    onClick={addNewTab}
                    className="bg-white text-[#2e7d32] hover:bg-[#e8f5e9] px-2 py-1 rounded-t-lg text-xs font-medium flex items-center border border-[#2e7d32]/30 border-b-0"
      ...
      {currentTab && (
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-1">
                  <h3 className="text-sm font-medium text-gray-900 flex items-center">
                    <svg className="w-4 h-4 mr-1 text-[#2e7d32]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              {currentTab.products.length === 0 ? (
                  ...
                  <table className="w-full">
                      ...
                      <tr>
                          ...
                          <th className="p-1 text-left text-[10px] font-medium">Biến thể</th>
                          <th className="p-1 text-center text-[10px] font-medium">SL</th>
                            ...
                            {momoQrString ? (
                              ...
                              <div className="flex items-center justify-center w-full h-full">
                                <div className="text-center">
                                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#b0006d] mx-auto mb-1"></div>
                                  <p className="text-gray-500 text-[10px]">Đang tải mã QR...</p>
                                </div>
                              </div>
                            )}
                          </div>
        ...
        {/* Product drawer */}
        <div
          className={`fixed top-[65px] left-0 bottom-[70px] w-2/3 bg-white shadow-2xl z-30 transform transition-transform duration-300 ease-in-out overflow-hidden
            ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}
        >
          <div className="flex flex-col h-full">
            <div className="flex justify-between items-center p-2 border-b border-[#2e7d32]/20">
              <h3 className="text-base font-semibold text-[#2e7d32] flex items-center">
                <svg className="w-5 h-5 mr-2 text-[#2e7d32]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                </svg>
                Danh sách sản phẩm
              </h3>
              <button
                className="text-gray-500 hover:text-[#2e7d32] bg-[#e8f5e9] rounded-full p-1.5"
      ...
      {showMomoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-auto">
          <div className="bg-white rounded-lg p-4 max-w-md w-full animate-fadeIn my-4 mx-2 shadow-xl">
            ...
            <div>
              ...
              <div className="bg-[#b0006d]/10 p-2 rounded-lg mb-3 max-h-32 overflow-y-auto">
                <p className="text-sm text-center text-gray-700 font-medium mb-1">Hướng dẫn thanh toán:</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 max-w-sm animate-fadeIn">
            ...
            <div className="mb-4">
              <div className="flex justify-center mb-3">
                <div className="w-16 h-16 rounded-full bg-[#e8f5e9] flex items-center justify-center">
                  <IoCheckmark className="text-[#2e7d32] w-10 h-10" />
                </div>
              </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={() => setShowInvoiceModal(false)}
                className="w-full py-2 bg-gray-200 text-gray-700 rounded font-medium hover:bg-gray-300 text-sm"
              >
                Đóng
              </button>
            </div>
      ...
      {/* Add an "X" button to close when drawer is open (visible on mobile) */}
      {isDrawerOpen && (
        <button
          className="fixed top-[60px] right-4 z-40 bg-[#2e7d32] text-white rounded-full p-1 shadow-md hover:bg-[#1b5e20] lg:hidden"
          onClick={() => setIsDrawerOpen(false)}
        >
          <IoClose size={18} />
        </button>
