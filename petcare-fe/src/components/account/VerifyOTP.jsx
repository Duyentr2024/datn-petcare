...
const VerifyOTP = () => {
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            console.log("🔍 Email:", email, "🔍 OTP:", otpCode, "🔍 OTP Type:", otpType);

            let response;
            if (otpType === "REGISTRATION") {
                response = await otpService.verifyRegistrationOtp(email, otpCode);
            } else{
                response = await otpService.verifyResetPasswordOtp(email, otpCode);
            }


            if (
                response === "Tạo tài khoản thành công!" ||
                response === "OTP hợp lệ. Bạn có thể đặt lại mật khẩu."
            ) {
                Swal.fire({
                    icon: "success",
                ...
                setTimeout(() => {
                    navigate(otpType === "REGISTRATION" ? "/login" : "/resetPassword", {state: {email}});
                }, 2000);
            } else {
                Swal.fire({
                    icon: "error",
                    title: response || "Có lỗi xảy ra!",
            ...
            Swal.fire({
                icon: "error",
                title: error.response?.data || "Đã xảy ra lỗi.",
                showConfirmButton: true,
            });
        } finally {
            setLoading(false);
    ...
    const handleResendOtp = async () => {
        ...
        setLoading(true);
        try {
            await otpService.resendOtp(email, otpType);
            Swal.fire({
                icon: "info",
