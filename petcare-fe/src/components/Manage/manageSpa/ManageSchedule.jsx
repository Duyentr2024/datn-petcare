...
const ManageSchedule = () => {
    ...
    // Get minimum allowed datetime (current time + 5 minutes)
    ...
    const fetchSchedules = async () => {
        try {
            setIsLoading(true);
            setErrorMessage(null);
            const response = await fetch('/api/staff/scheduled-booking-changes', {
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Không thể tải danh sách lịch');
            }
    ...
    const confirmSubmit = async () => {
        try {
            ...
            setErrorMessage(null);
            const url = isEditing 
                ? `/api/staff/scheduled-booking-changes/${selectedSchedule.id}`
                : '/api/staff/schedule-booking-enabled';
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || (isEditing ? 'Không thể cập nhật lịch' : 'Không thể tạo lịch mới'));
            }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            setIsLoading(true);
            setErrorMessage(null);
            const response = await fetch(`/api/staff/scheduled-booking-changes/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Accept': 'application/json'
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.message || 'Không thể xóa lịch');
            }
            await fetchSchedules();
        } catch (error) {
            setErrorMessage(error.message);
