...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        setLoading(true);
        const data = await ProductDetailService.getProductDetailsDTOByProductId(
          productId
        );
  ...
  const handleSizeChange = (size) => {
    ...
    setTimeout(() => {
      ...
      updateProductDetailBasedOnSelection(
        newSize,
        selectedColor,
        selectedWeight
      );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border-l-[1px] border-l-amber-500">
            ...
            {product?.imageUrls?.map((url, index) => (
              <img
                ...
                className={`w-12 h-12 rounded-xl cursor-pointer border-2 border-gray-300 hover:ring-2 ring-yellow-500 transition transform hover:scale-105 ${
                  mainImage === url ? "ring-2 ring-yellow-500" : ""
                }`}
              />
            ))}
          </div>
        ...
        <div className="flex flex-col space-y-4">
            ...
            <div className="flex space-x-2">
              {product?.availableSizes?.map((size) => (
                <button
                  ...
                  disabled={
                    !isValidCombination(size, selectedColor, selectedWeight)
                  }
            ...
            <div className="flex space-x-2">
              {product?.availableWeights?.map((weight) => (
                <button
                  ...
                  disabled={
                    !isValidCombination(selectedSize, selectedColor, weight)
                  }
                  className={`px-3 py-1 border rounded-full text-sm ${
            ...
            <div className="flex space-x-2">
              {product?.availableColors?.map((color) => (
                <button
                  ...
                  disabled={
                    !isValidCombination(selectedSize, color, selectedWeight)
                  }
                  className={`px-3 py-1 border rounded-full text-sm ${
                    selectedColor === color
                      ? "bg-yellow-100 text-yellow-600 font-semibold"
          ...
          <div className="flex space-x-3">
            <Link to="/checkout">
              <button className="px-5 py-2 bg-yellow-500 text-white font-semibold rounded-xl hover:bg-yellow-600">
                Thanh toán
              </button>
            </Link>
